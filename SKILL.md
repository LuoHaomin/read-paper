...
- **MinerU 非零误差**：偶有符号识别错（如把某算符误识成 `\mathsf{1}`），图片区可能输出空行。故关键公式仍走 §3 **抽查闸**（不必逐条）。

**回退：`extract.sh` / `pdftotext`（无 MCP 或 MCP 失败时）。** 脚本在本 SKILL.md 同目录，`-layout` 已内置（双栏必须）：
...
此路径得到的是 **Unicode 残缺体**，公式须走 §3 视觉闸**逐条**重建。

> **抽取来源（MinerU Markdown vs pdftotext 文本）决定 §3 视觉闸的严格程度**——记住你这次走的是哪条。
...
2. **视觉闸（按抽取来源分档）** ——
   - **源 = MinerU Markdown**（公式已是 LaTeX）：**降级为抽查**。MinerU 已把方程转成 LaTeX，只在某公式读起来可疑 / 与上下文矛盾 / 含生僻符号时，才用 Read 把对应 PDF 页当图读核对；其余直接采信。不是零核对（MinerU 偶错符号），但不必逐条。
   - **源 = pdftotext 文本**（Unicode 残缺体）：**逐条视觉核对**。凡要从 Unicode 重建的公式，先用 Read 把对应 PDF 页当图读核对（`Read(file_path=<PDF>, pages="5")`），视觉读方程 >> 读残缺文本，确认后再写成规范 LaTeX。
   - **识图不可用时降级，绝不阻塞流程**：若按页读图 / 识图工具不可用（限流 429、工具缺失、图片糊到读不出），**不要反复重试、也不要停摆**。改为：对抽取文本 + 上下文尽力重建公式，**把拿不准的精确形式（下标/上标、归一化常数、指数项、记号约定）就地标 `⚠️ 待核：需人工比对 p.X`**；能从文本确定的整体结构照常写。收尾在 README 汇总全部待核点，交人工一次性核对。原则——**有疑虑的细节一律标待核，而非猜测**。
3. **不补全闸** —— 论文没有的章节（如局限、未来工作）就写"原文未讨论"。**禁止替作者编造局限、数字或引用。**

收尾**抽查**：每个条目写完，随机回原文核对 2 处公式/数字。
...
- **Unicode → LaTeX**：抽出来的文本只当"草稿"，最终公式读懂后用规范 LaTeX 重写（`$\rho_s$`、`$|\partial A|$`、`$\leq$`）。不确定的走 §3 视觉闸。
...
- [ ] 公式来源分档处理：**MinerU 源**抽查可疑公式、**pdftotext 源**逐条视觉核对（Read 按页读 PDF 图）？（见 §3）
...
- **Unicode 数学残缺体。** `Ω`→`Ω`、`∣`→`|`、`∶`→`:`、`ρs`→`$\rho_s$`、`≤`→`$\leq$`。只当草稿，最终 LaTeX 自己重写（§3 视觉闸）。
- **方向性不等式极易写反。** 凡出现"≥ 还是 ≤"的步骤，都在"为什么非平凡"里把这方向单独讲清；拿不准翻原文 + Read 工具看图核对。
...
| 症状 | 原因 / 修复 |
|------|-----------|
| `mcp__mineru__parse_documents` 不可用 | MCP 没连。`/mcp` 看 `mineru` 是否 connected；没配就 `claude mcp add mineru -- uvx mineru-open-mcp`（带 token 走 precision）。临时走 §1 的 pdftotext 回退。 |
| MinerU 解析超时 / 失败 | 多半网络或额度。重试一次；仍失败就走 pdftotext 回退，别阻塞。 |
| MinerU 把某符号识别错（如 `\mathsf{1}`） | 正常，非零误差。对该公式用 Read 读 PDF 原页核对，或标 `⚠️ 待核`。 |
...
| `pdftotext: command not found` | 没装 poppler。`brew install poppler` / `apt-get install poppler-utils`。 |
...
| 某公式怎么都看不懂 | 用 Read 工具把那一页当图读：`Read(file_path=PDF, pages="5")`。 |
...
```
MinerU MCP: parse_documents(paper.pdf)   # 1. 取文本（首选 MinerU→干净 LaTeX；无 MCP 则 extract.sh/pdftotext 兜底）
分诊：判类型 + 拆解计划                    # 2. 理论?实验?综述?系统? → 写哪些承重单元
立防幻觉三闸（按来源分档）                  # 3. 溯源 / 视觉核对(MinerU 抽查·pdftotext 逐条) / 不补全
