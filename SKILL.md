---
name: read-paper
description: 精读并拆解学术论文（以 PDF 为主），自动适配论文类型（理论证明 / 实验方法 / 综述 / 系统）。把一篇或多篇论文整理成一套详细、可教学的分析条目（每个承重结果 / 方法 / 实验一个 markdown 文件）外加一份快速导读（导读.md，30 分钟读懂核心思路）与导航总览（README.md）：条目含形式陈述或问题动机、大白话直觉、分步推导或实验解读、非平凡之处、以及它在全局里的角色；公式用规范 LaTeX，关系图用 mermaid、几何图用内联 SVG。当用户要求 读论文 / 解读 / 拆解 / 整理 / 分析 / 精读 / summarize / analyze / digest 一篇或多篇论文、PDF、或要把论文变成学习笔记时使用。
---

# read-paper — 论文精读拆解

把一篇（或多篇）学术论文整理成一套**详细、可教学的分析条目 + 一份快速导读（`导读.md`）与导航总览（`README.md`）**。本 skill **自动识别论文类型**（理论证明 / 实验方法 / 综述 / 系统），按类型选用对应的拆解模板——不再把所有论文都套进"定理+证明"的模子。
...
把计划（类型 + 条目清单）简短列出来，再开写。顺手记下 2–5 个候选"**关键想法**"（§5 写导读时用；条目写完后复核、蒸馏——想法 = 删掉它论证就断的那一环）。
...
## 5. 写总览：导读.md + README.md（每篇必出两个）

**所有条目写完之后**，收束成两个互补的文件（放该论文输出文件夹根目录）：

- **`README.md` = 地图（导航中枢）**：结果/贡献地图 + 骨架图 + 每条一句话定位 + 互链。照 [`reference/overview-template.md`](reference/overview-template.md) 填。其"建议阅读顺序"**固定把导读列为第 0 步**。
- **`导读.md` = 导游（教学叙事）**：问题 → 2–5 个关键想法（每个 = 一句标题 + 大白话观察 + 1–3 个公式 + 易错点 + 链接条目）→ 它们怎么咬合 + 一图流 + 原文快速阅读路线 + 导读→条目对照表。照 [`reference/guide-template.md`](reference/guide-template.md) 填。目标是**让读者 30 分钟读懂核心思路，再回原文时各节都是填空题**。

先后顺序：先 README 后导读（导读是对**全部条目**的蒸馏，要等条目齐了才能写、才能链接）。这一步把零散条目收束成全局视野——读者从任意文件进来，都能顺着这对入口摸清全篇。
...
```
<输出目录>/<论文标题>/
  导读.md           ← 必出：教学叙事（问题 → 关键想法 → 咬合 + 一图流 + 对照表）
...
- [ ] **导读.md 已产出**：问题 → 2–5 个想法 → 咬合，大白话、公式限量（≤10 个 display）、每个想法链接条目、末尾有导读→条目对照表？（见 §5 与 guide-template.md）
