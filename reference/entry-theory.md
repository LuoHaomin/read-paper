...
> **防幻觉铁律**（写之前先看 SKILL.md §3）：形式陈述、每步用到的前置结果、每个公式都要能挂回原文 `(p.X)` / `Eq.(N)` / 直接引用；挂不上就标 `⚠️ 待核`，绝不编造；Unicode 残缺公式从文本 + 上下文重建为 LaTeX，拿不准同样标 `⚠️ 待核`（不做视觉核对，见 SKILL.md §3 重建闸）。
...
- 行间公式 `$$...$$`，行内 `$...$`；核心结论用 `\boxed{}`。
- **公式一律规范 LaTeX**，绝不照抄 `pdftotext` 的 Unicode 残缺体（`Ω`→`Ω`、`∣`→`|`、`∶`→`:`、`ρs`→`$\rho_s$`、`≤`→`$\leq$`）。不确定的公式从文本 + 上下文重建，拿不准标 `⚠️ 待核`（不做视觉核对，见 SKILL.md §3 重建闸）。
