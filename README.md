...
精读并拆解学术论文（以 PDF 为主），**自动适配论文类型**（理论证明 / 实验方法 / 综述 / 系统）。把一篇或多篇论文整理成一套详细、可教学的分析条目（每个承重结果 / 方法 / 实验一个 markdown 文件），外加一份**快速导读**（`导读.md`：问题 → 关键想法 → 咬合，30 分钟读懂核心思路）与**导航总览**（`README.md`）。
...
条目之上收束成两个入口：`导读.md` 是**导游**（教学叙事，按读者的理解顺序讲清论文靠哪几个想法站起来），`README.md` 是**地图**（贡献地图、骨架图、互链导航）。
